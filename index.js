...
function updateCollisionIndicator(show, playerPosition, id, type) {
    if (show) {
        ...
        if (collisionIndicators[type][id]) {
            ...
            indicator.style.left = `${canvasRect.left + playerPosition.x + player.width / 2
                }px`;
        ...
        if (e.key === "e" || e.key === "E") {
            ...
            chestIds.forEach((chestId) => {
                ...
                if (chestDiv && chestId === divId) {
                    const newDisplayState =
                        chestDiv.style.display === "none" ? "block" : "none";
...
class Chatbot {
  ...
  async initializeModel() {
    try {
      // Load the UMD version of the transformers library
  ...
  async sendMessage() {
    ...
    if (this.isModelLoading) {
      this.appendMessage("Please wait while the AI model is loading...", "bot");
      return;
    }
    ...
    try {
      ...
      this.appendMessage("I'm having trouble understanding right now. Could you try again?", "bot");
    }
  }
  ...
  async generateAIResponse(message) {
    try {
      ...
      const sentiment = await this.pipeline(message);
      ...
      // Basic response logic with sentiment awareness
      const lowercaseMsg = message.toLowerCase();

      if (lowercaseMsg.includes('hi') || lowercaseMsg.includes('hello')) {
        return "Hi langga ni kaon naka ara?";
      }

      if (lowercaseMsg.includes('wala')) {
        return "Kaon na langga ayaw sig code kay MABOANG naka ana";
      }

      if (lowercaseMsg.includes('name')) {
        return "I'm an AI-powered chatbot. What's your name?";
      }

      if (lowercaseMsg.includes('weather')) {
        return "I'm not sure about the weather right now, but you can check a weather app!";
      }

      // Use sentiment for dynamic responses
      if (sentimentLabel === 'POSITIVE' && score > 0.8) {
        return "I can tell you're feeling very positive! That's wonderful. How can I help keep that energy going?";
      } else if (sentimentLabel === 'POSITIVE') {
        return "I sense some good vibes! How can I help you further?";
      } else if (score < 0.3) {
        return "I notice you might be feeling down. Would you like to talk about what's bothering you?";
      } else {
